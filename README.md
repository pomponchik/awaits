...
Этот декоратор проще, чем [```@awaitable```](#декоратор-awaitable). Обернутая им функция будет просто "отстрелена" в группу потоков, без ожидания результата. Возвращен при это будет объект класса [Task](#об-объекте-задачи), что позволяет вручную отслеживать статус выполнения.
