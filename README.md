...
@awaitable
def sum(a, b):
  # Какой-то сложный датасаенз. Что-то, что вычисляется долго и мешает вашему event-loop'у.
  return a + b

# Теперь sum - это корутина! Пока она выполняется, управление передается в event-loop.
asyncio.run(sum(2, 2))
```
